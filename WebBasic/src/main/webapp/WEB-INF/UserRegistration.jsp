...
<%
    String jsonString = (String) request.getAttribute( "reg-errors" ) ;
    String regMessage = (String) request.getAttribute( "reg-message" ) ;

    String miss_all_err = null;
    String user_login_err = null;
    String user_pass_err = null;
    if(jsonString != null) {
        JSONObject errors = new JSONObject(jsonString);
        miss_all_err = errors.has("miss-all-par") ? errors.optString("miss-all-par") : null;
        user_login_err = errors.has("user-login") ? errors.optString("user-login") : null;
        user_pass_err = errors.has("user-pass") ? errors.optString("user-pass") : null;
    }
%>
<% if( regMessage != null && "OK".equals( regMessage ) ) { %>
<h4>Реєстрація успішна.</h4>
<% } else if (miss_all_err != null) { %>
<h4>Помилка реєстрації: <%= miss_all_err %> </h4>
<% } else { %>
<h4>Введіть облікові дані:</h4>
<% } %>
<%--<% } else { %>--%>
<%--<h4>Помилка реєстрації: <%= user_login_error %> </h4>--%>
<%--<% } %>--%>

<div class="row">
    <form class="col s10 offset-s1 m8 offset-m2 l6 offset-l3" method="post" enctype="multipart/form-data">
        <% if(user_login_err != null) { %>
        <p><%= user_login_err %></p>
        <% } %>
        ...
        <div class="row input-field right-align">
            <button class="btn waves-effect waves-teal" type="submit">REGISTER<i class="material-icons right">exit_to_app</i></button>
        </div>
    </form>
</div>